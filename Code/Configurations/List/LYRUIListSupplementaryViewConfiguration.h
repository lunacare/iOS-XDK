//
//  LYRUIListSupplementaryViewConfiguration.h
//  Layer-UI-iOS
//
...
@interface LYRUIListSupplementaryViewConfiguration<SupplementaryViewType> : NSObject <LYRUIListSupplementaryViewConfiguring,
...
/**
 @abstract Supplementary view setup block, called inside `setupSupplementaryView:forItemAtIndexPath:`. The view, list data source, and index path are passed to the block, to setup the reusable view (or it's subview) with the data from model object, using view configuration.
 */
...
/**
 ...
 @param reuseIdentifier Reuse identifier of the supplementary view used by the configuration to present model data.
 ...
 @returns An instance of `LYRUIListSupplementaryViewConfiguration` ready for use in `LYRUIListDelagate` and `LYRUIListDataSource`.
 */
