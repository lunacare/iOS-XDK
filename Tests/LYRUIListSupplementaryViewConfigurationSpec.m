...
SpecBegin(LYRUIListSupplementaryViewConfiguration)

describe(@"LYRUIListSupplementaryViewConfiguration", ^{
    __block LYRUIListSupplementaryViewConfiguration *viewConfiguration;
    ...
    beforeEach(^{
        ...
        viewConfiguration = [[LYRUIListSupplementaryViewConfiguration alloc] initWithSupplementaryViewClass:[UICollectionReusableView class]
                                                                                                       kind:@"test kind"
    ...
    it(@"should return `test kind` as handled view kind", ^{
        expect(viewConfiguration.viewKind).to.equal(@"test kind");
    });
    it(@"should return `test reuse identifier` as view reuse identifier", ^{
        expect(viewConfiguration.viewReuseIdentifier).to.equal(@"test reuse identifier");
    });
    ...
    describe(@"supplementaryViewSizeInCollectionView:forItemAtIndexPath:", ^{
        ...
        context(@"when view should be visible", ^{
            beforeEach(^{
                ...
                returnedSize = [viewConfiguration supplementaryViewSizeInCollectionView:collectionViewMock forItemAtIndexPath:indexPath];
            });
    ...
    describe(@"setupSupplementaryView:forItemAtIndexPath:", ^{
        ...
        beforeEach(^{
            ...
            viewConfiguration.listDataSource = dataSourceMock;
            
            [viewConfiguration setupSupplementaryView:viewMock forItemAtIndexPath:indexPath];
        });
    ...
    describe(@"registerSupplementaryViewInCollectionView:", ^{
        context(@"when initialized with view registration block", ^{
            beforeEach(^{
                [viewConfiguration registerSupplementaryViewInCollectionView:collectionViewMock];
            });
