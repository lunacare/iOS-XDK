...
- (UIView *)viewForMessage:(LYRUIMessageType *)message {
    Class<LYRUIMessageViewContainer, LYRUIStandardMessageContainerViewTheme, LYRUIConfigurable> containerViewClass = [[self class] containerViewClass];
    LYRUIStandardMessageContainerView *container = [self.reusableViewsQueue dequeueReusableViewOfType:containerViewClass];
    if (container == nil) {
        container = [[(Class)containerViewClass alloc] init];
...
{
    return [LYRUIStandardMessageContainerView class];
}
